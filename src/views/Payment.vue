...
export default {
    ...
    created() {
        this.$store.dispatch("model/sendReq", 
                {method: "all", url: 'user/deposit/address/'})

        if (this.$route.params.slug){
            this.$store.dispatch("model/sendReq", 
                {method: "get", url:`meeting`, id: this.$route.params.slug })
        } else {
            this.$store.dispatch("model/sendReq", 
                {method: "get", url: 'course', id: this.$route.params.id })
        }
    }
}
</script>
