<template>
    <v-card v-bind='$attrs' >
                        ...
                        >
                            <v-row justify='start' align='end' class='fill-height'>
                                <v-col cols=6  class="mr-2">
                                    <v-btn color="red" dark style="font-size:12px" elevation=0>
                                        {{ data.section }}
                                    </v-btn>
                                </v-col>
                            </v-row>
        ...
        <v-card-text>
            ...
            <v-row 
                v-if='data.teacher_profile'
                align="center"
                class="mx-0">
                <v-col cols=12 class="mx-0 ">
                    <v-avatar tile color="#333" class="rounded-lg float-left">
                   </v-avatar>
                   <h6 > معلم الكورس</h6>
               ...
               <v-col cols=12>
                    ...
                    <v-rating
                        ...
                        size="16"
                    ></v-rating>
                    <div class="rating-number grey--text ml-4">
                        {{data.ratings.rating__avg}} ({{data.ratingCount}})
                    </div>
               </v-col>
            </v-row>
        </v-card-text>  
            ...
            <v-row justify='center'>
                <v-col cols=12>
                    <v-btn 
                        ...
                        elevation=0 
                        class="ml-1"
                        @click="$router.push({name: 'Payment', params: {id: data.id} })"
                    > 
                        إشتري 
                    </v-btn>
                    <v-btn  v-else
