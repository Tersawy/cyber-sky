        ...
        <v-container >
            <v-layout row wrap  justify-center style='top: -20pt;position: relative;'>
                <v-col md=4 cols=12 v-for='course in courses.results' :key="course.id">
                    <CoursesCard :data='course' :class='`mt-${course.id * 4}`'>
                    </CoursesCard>
                </v-col>
            </v-layout>
            ...
            <v-layout row wrap  justify-center >
                <v-col cols=12>
                    <h1 
                        class="text-h2 text-center ma-4 pa-4"
...
export default {
    ...
    created() {
        ...
        this.$store.dispatch("model/sendReq", 
                {method: "all", url: 'course', params: {limit:3}})
                .then( resp=>{
                    // console.log(resp.data.results)
                    this.courses = resp.data
                })
        this.$store.dispatch("model/sendReq", 
                {method: "all", url: 'blog/post', params: {limit:2}})
                .then( resp=>{
